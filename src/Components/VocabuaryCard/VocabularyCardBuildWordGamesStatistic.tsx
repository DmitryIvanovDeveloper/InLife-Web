...
export default function VocabularyCardBuildWordGamesStatistic(props: IVocabilaryCardQuizlGameStatistic) {
    const theme = useTheme();
    const fullScreen = useMediaQuery(theme.breakpoints.down('md'));

    const [isOpenStatisticTable, setIsOpenStatisticTable] = useState<boolean>(false);
    const [buildWordGamesBySpecificWord, setBuildWordGamesBySpecificWords] = useState<IBuildWordsGameStatistic[]>([]);
    const [correctAnswers, setCorrectAnswers] = useState<number>(0);
    const [unCorrectAnswers, setUncorrectAnswers] = useState<number>(0);

    useEffect(() => {
        const wordGameStatistic = props.buildWordsGameStatistic.filter(statistic => statistic.wordId == props.wordId);
    ...
    useEffect(() => {
        ...
        buildWordGamesBySpecificWord.forEach(statistic => {
    ...
    function CardQuizleGameTable() {
        return (
            ...
            >
                <DialogContent >
                    <DialogContentText>
                        <BuildWordGameTable buildWordsGameStatistics={buildWordGamesBySpecificWord} />
                    </DialogContentText>
                </DialogContent>
                <DialogActions>
